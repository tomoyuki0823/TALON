...
public class Pattern2Strategy implements YotakukinPatternStrategy {

    @Override
    public void apply(Map<String, Object> map, YotakukinContext ctx) {

        if ("90".equals(ctx.honTaisyoku) || "91".equals(ctx.honTaisyoku)) {
            map.put("HON_YOTAKUKIN_KINGAKU", BigDecimal.ZERO);
            map.put("HON_TYOIKIN_KINGAKU", BigDecimal.ZERO);
        } else {
            map.put("HON_YOTAKUKIN_KINGAKU", ctx.haiYotakukin.subtract(ctx.haiYotakukinShiharai));
            map.put("HON_TYOIKIN_KINGAKU", BigDecimal.valueOf(5000));
        }
        map.put("HAI_YOTAKUKIN_KINGAKU", "99".equals(ctx.haiTaisyoku) ? ctx.haiYotakukin.subtract(ctx.haiYotakukinShiharai) : BigDecimal.ZERO);
